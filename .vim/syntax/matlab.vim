...
"
"		Edited locally by: Marc Joliet
"
" Notes by Marc Joliet:
"
" I modified this syntax file to fold conditionals and for loops, because I really hated not being
" able to. I also added folding for block comments and some syntax errors.
"
...
" don't spell-check by default, only in regions containing @Spell
...
syn region matlabMultilineComment	start="%{" end="%}.*$" contains=matlabTodo,matlabTab,@Spell fold

syn region matlabConditionalRegion	matchgroup=matlabConditional start="^\s*if\s*"me=s end="^\s*end\s*;\?$" end=";\s*end\s*;\?" contains=ALL transparent fold
syn region matlabLabelRegion		matchgroup=matlabLabel start="^\s*switch\s\+" end="^\s\*end\s*$" contains=ALL transparent fold
syn region matlabRepeatRegion		matchgroup=matlabRepeat start="^\s*\(for\|while\)\s\+" end="^\s*end\s*$" contains=ALL  transparent fold

syn match matlabBlockFoldErr		"^\s*\(if\|for\|while\|switch\)\s*$" contains=matlabTab
...
if version >= 508 || !exists("did_matlab_syntax_inits")
  ...
  HiLink matlabBlockFoldErr		Error
